...
* `DEBUG`: Указывает, должен ли Django использовать режим отладки.
   > Пример: `True` или `False`. По умолчанию `True`.

* `ALLOWED_HOSTS`: Список хостов/доменов, на которых будет работать ваш проект Django.
   > Пример: `your-allowed-hosts`. По умолчанию `127.0.0.1`.

### Настройки логирования (необязательно)

* `LOGURU_FOLDER`: Путь до папки для журналов Loguru.
   > Пример: `"your-loguru-folder"`. По умолчанию `logs`.

* `LOGURU_LEVEL`: Уровень логирования для Loguru.
   > Пример: `"your-loguru-level"`. По умолчанию `DEBUG`.
...
* `DB_ENGINE`: Движок базы данных Django.
   > Пример: `"your-db-engine"`. По умолчанию `django.db.backends.mysql`.

* `DB_NAME`: Имя вашей базы данных.
   > Пример: `"your-db-name"`. По умолчанию `cars`.

* `DB_USER`: Имя пользователя для доступа к вашей базе данных.
   > Пример: `"your-db-user"`. По умолчанию `root`.

* `DB_PASSWORD`: Пароль для доступа к вашей базе данных.
   > Пример: `"your-db-password"`. По умолчанию `root`.

* `DB_HOST`: Хост вашей базы данных.
   > Пример: `"your-db-host"`. По умолчанию `localhost`.

* `DB_PORT`: Порт вашей базы данных.
   > Пример: `your-db-port`. По умолчанию `3306`.

### Настройки очереди заданий Celery (необязательно)

* `CELERY_BROKER_URL`: URL брокера для Celery.
   > Пример: `"your-celery-broker-url"`. По умолчанию `redis://localhost:6379`.

### Настройки Telethon (необязательно)

* `TELETHON_SESSION_NAME`: Имя сессии для Telethon.
   > Пример: `"your-telethon-session-name"`. По умолчанию `session`.

* `TELETHON_SYSTEM_VERSION`: Версия системы для Telethon.
   > Пример: `"your-telethon-system-version"`. По умолчанию `4.16.30-vxCUSTOM`.

### Другие необязательные настройки

* `LOGIN_URL`: URL для входа в систему.
   > Пример: `"your-login-url"`. По умолчанию `/users/login/`.

* `ANNOUNCEMENT_LIST_PER_PAGE`: Количество объявлений на странице.
   > Пример: `your-announcement-list-per-page`. По умолчанию `5`.

* `TAG_LIST_PER_PAGE`: Количество тегов на странице.
   > Пример: `your-tag-list-per-page`. По умолчанию `10`.
...
5. **Запуск проекта:**
   Если вы не захотели запускать проект с помощью скрипта, вы можете сделать это вручную:
