        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status.toLowerCase().startsWith("опубликовано")) {
                    ...
                    copyButton.show();
                    console.log("Show takeoff button");
                    row.find(".takeoff-button").show();
                } else {
                    console.log("Hide copy button");
                    copyButton.hide();
                    console.log("Hide takeoff button");
                    row.find(".takeoff-button").hide();
                }
                ...
                if (data.status.toLowerCase().startsWith("опубликовано")) {
                    statusIcon.removeClass('status-awaiting status-removed');
                    statusIcon.addClass('status-published');
                } else if (data.status.toLowerCase().startsWith("ожидает публикации")) {
                    statusIcon.removeClass('status-published status-removed');
                    statusIcon.addClass('status-awaiting');
                } else if (data.status.toLowerCase().startsWith("снято с публикации")) {
                    statusIcon.removeClass('status-published status-awaiting');
                    statusIcon.addClass('status-removed');
                }
